...
contract RewardFacet is Modifiers {
  ...
  * @param timeAmount (timeAhead*amountStaked) the product of the amount staked and how long.  Used to to determine what proportion the user receives from the penalty pool 
   */
   ...
   * @param startTime the time the position started accruing sdex block rewards
   * @param endTime the time the position was commited to
  ...
  /** 
   * Returns a list of addresses belong to the valid NFT's that can be minted for a specific token
  ...
  /**
   * Returns a list of nft addresses that can be applied to a specific pool


  function getValidNFTsForPool(uint256 pid) public returns (address[] memory) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    //return s.validNFTsForPool;
  }
