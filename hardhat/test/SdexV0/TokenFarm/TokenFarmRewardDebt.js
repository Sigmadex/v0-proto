...
function logState(state, tag, alice, diamondAddress) {
  ...
  console.log('state   ')
  console.log('--------')
  console.log('trwddebt', BN(state[alice].userInfo.tokenData[0].totalRewardDebt).div(unity).toString())
...
contract("SdexVaultFacet Many Deposits", (accounts) => {
  ...
  it("stakes many times to Vault Separately", async () => {
    ...
    await tokenFarmFacet.methods.deposit(
      0, [stakeAmount], blocksToStake, ADDRESSZERO, 0).send({from:alice})
    ...
    await tokenFarmFacet.methods.withdraw(0,0).send({from: alice})
