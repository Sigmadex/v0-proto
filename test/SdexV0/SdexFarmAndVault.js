...
contract("Sdex Farm and Vault Together", async (accounts) => {
  ...
  it("one user stakes vault, another stake manual", async () => {
    ...
    console.log(BN(state5[diamondAddress].sdex).sub(
      ...
      BN(state5.accSdexRewardPool))).toString()
    )
    ...
    assert.equal(vUserInfo2.positions[0].timeEnd - vUserInfo2.positions[0].timeStart, blocksToStake);
    ...
    //Token Globals
    assert.equal(tokenGlobal2.timeAmountGlobal, blocksToStake*stakeAmount)
