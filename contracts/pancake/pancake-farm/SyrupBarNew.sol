...
import "./MasterChefNew/ACL.sol";
// SyrupBar with Governance.
contract SyrupBarNew is BEP20('SyrupBar Token', 'SYRUP') {
  /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ACL).
  function mint(address _to, uint256 _amount) public onlyACL {
    _mint(_to, _amount);
    _moveDelegates(address(0), _delegates[_to], _amount);
  }

  function burn(address _from ,uint256 _amount) public onlyACL {
    _burn(_from, _amount);
